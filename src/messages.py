...
class AgentMessage(BaseModel):
    ...
    message_type: Union[Literal[MessageType.CMD_RESPONSE], Literal[MessageType.CMD_REQUEST]]
    ...
    def validate_data(cls, v: str | bytes):
        ...
        If the incoming data is `bytes`, keep it as `bytes`.
        """
        if isinstance(v, str):
            try:
    ...
    @classmethod
    def from_encoded_file(cls, encoded_filepath: Path, decoded_dir: Path, decoded_suffix: str = ".data") -> "AgentMessage":
        """
        ...
        out_path = (decoded_dir/Path(decoded_filename)).resolve()
        # Write result out to disk (since we can't hold it in memory)
        dddb.video.decode({'in_path': encoded_filepath.resolve(), 'out_path': out_path})
        
        # Grab result back from disk, interpret as JSON
        with open(out_path, "rb") as fp:
            return cls.model_validate_json(fp.read())
    
    def to_encoded_file(self, decoded_dir: Path, encoded_dir: Path, suffix: str = ".mp4") -> Path:
        """
        ...
        """
        decoded_out_path = (decoded_dir/(Path(self.msg_id).with_suffix('.data'))).resolve()
        encoded_out_path = (encoded_dir/Path(self.msg_id+suffix)).resolve()
        if decoded_out_path.exists():
            raise RuntimeError(f"A message at {decoded_out_path} already exists!")
        
        logger.debug(f"Writing the following decoded message to {decoded_out_path}: {self}")
        ...
        dddb.video.encode({'in_path': decoded_out_path, 'out_path': encoded_out_path})
